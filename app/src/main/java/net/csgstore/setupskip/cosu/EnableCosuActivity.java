...
/**
 * This activity is started after provisioning is complete in {@link AdminReceiver} for
...
public class EnableCosuActivity extends Activity {
    ...
    private static final String TAG = "EnableCosuActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (persistableBundle == null) {
            Log.e(TAG, "No admin extra bundle");
            finish();
            return;
        }
        ...
        registerReceiver(mInstallReceiver, new IntentFilter(PackageInstallationUtils.ACTION_INSTALL_COMPLETE));

        // download the config file
        String configDownloadLocation = (String) persistableBundle.get(BUNDLE_KEY_COSU_CONFIG);
        if (configDownloadLocation == null) {
            Log.e(TAG, "No download-location specified");
            finishWithFailure();
            return;
        }
        mConfigDownloadId = CosuUtils.startDownload(mDownloadManager, mHandler,
    ...
    private BroadcastReceiver mDownloadReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            mHandler.sendMessage(mHandler.obtainMessage(CosuUtils.MSG_DOWNLOAD_COMPLETE,
                    new Long(id)));
        }
    };
